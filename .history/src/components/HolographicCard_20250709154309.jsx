...
export default function HolographicCard({
		...
		if (imageUrl) {
			...
			const texture = new THREE.TextureLoader().load(
				...
				() => {
					...
					// For rounded rectangles, we might need to adjust the texture mapping
	...
	const holographicMaterial = useMemo(() => {
		if (!cardTexture) {
			console.log('No card texture available for material creation')
			return null
		}

		console.log('Creating holographic material with texture:', cardTexture)
		const material = new THREE.MeshStandardMaterial({
			map: cardTexture,
			transparent: true,
			opacity: 1.0, // Increased opacity
			metalness: 0.1, // Reduced metalness
			roughness: 0.8, // Increased roughness for better visibility
			emissive: new THREE.Color(hologramColor),
			emissiveIntensity: 0.1, // Slight emissive for visibility
		})
		...
		<group ref={cardRef} {...props}>
			...
			<group>
				...
				<group ref={meshRef}>
					...
					{/* Debug: Add a simple plane to test if the texture works */}
					{imageUrl && (
